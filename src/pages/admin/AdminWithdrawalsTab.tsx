...
import {
  ...
  checkUniVePayWithdrawalStatus,
} from '../../services/api';
import { WithdrawalItem, WithdrawalTransaction } from '../../types';
...
export const AdminWithdrawalsTab: React.FC<AdminWithdrawalsTabProps> = ({
  ...
  const [gatewayTxns, setGatewayTxns] = useState<WithdrawalTransaction[]>([]);
  ...
  const handleQueryGatewayStatus = async (traceno: string, amount: number) => {
    setReconcilingId(traceno);
    try {
      const res = await checkUniVePayWithdrawalStatus(traceno, amount);
      if (res.success) {
        onShowToast(`Gateway status: ${res.data?.data?.status || res.data?.status || 'UPDATED'}`);
        loadWithdrawals();
      } else {
        onShowToast('Gateway query failed. Please check credentials.');
      }
    } catch (e: any) {
      onShowToast(e.message || 'Failed to query gateway');
    } finally {
  ...
  const completedTotal = withdrawals.filter((w) => w.status === 'COMPLETED').reduce((acc, w) => acc + w.amount, 0);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              Withdrawal Queue & Gateway Disbursements
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Supports both UniVePay Automated Gateway Payouts and Manual Bank Disbursements with atomic locking and refunds.
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center gap-3">
          <div className="flex-1 w-full relative">
            ...
            <input
              ...
              placeholder="Search by Bank Account, IFSC, Holder Name, ID..."
            ...
            <tbody className="divide-y divide-gray-800/60">
              {loading ? (
                ...
                filteredWithdrawals.map((item) => {
                  ...
                  // Match with gateway transaction if available
                  const gwTxn = gatewayTxns.find(
                    (g) => g.id === item.id || g.traceno === item.referenceId || g.traceno === item.id
                  );
                    ...
                    <tr key={item.id} className="hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="py-3 px-4 font-mono text-[11px]">
                        <div className="flex items-center gap-1.5 mb-1">
                          {gwTxn?.method === 'UNIVEPAY_AUTO' ? (
                            <span className="px-1.5 py-0.5 rounded bg-emerald-950/80 border border-emerald-800/60 text-emerald-400 text-[10px] font-bold flex items-center gap-1">
                              <Zap className="w-3 h-3" /> UniVePay Auto
                            </span>
                          ) : (
                            <span className="px-1.5 py-0.5 rounded bg-amber-950/80 border border-amber-800/60 text-amber-400 text-[10px] font-bold flex items-center gap-1">
                              <Banknote className="w-3 h-3" /> Manual Payout
                            </span>
                          )}
                        </div>
                        <div className="text-white font-bold">{item.bankDetails?.holderName || 'N/A'}</div>
                        <div className="text-gray-400">
                          A/C: <span className="text-amber-400">{item.bankDetails?.accountNumber}</span> • IFSC: <span className="text-cyan-400">{item.bankDetails?.ifscCode}</span>
                        </div>
                        <div className="text-[10px] text-gray-500">
                          Bank: {item.bankDetails?.bankName} {item.bankDetails?.upiId ? `• UPI: ${item.bankDetails.upiId}` : ''}
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        ...
                        {gwTxn?.gatewaySerialNo && (
                          <div className="text-[9.5px] text-gray-400 font-mono mt-0.5">
                            SN: {gwTxn.gatewaySerialNo}
                          </div>
                        )}
                      </td>

                      <td className="py-3 px-4 text-right space-x-1.5 whitespace-nowrap">
                        {gwTxn?.method === 'UNIVEPAY_AUTO' && isPending ? (
                          <button
                            onClick={() => handleQueryGatewayStatus(gwTxn.traceno, gwTxn.amount)}
                            disabled={reconcilingId === gwTxn.traceno}
                            className="px-2.5 py-1.5 rounded-lg bg-cyan-950/60 border border-cyan-800 hover:bg-cyan-900 text-cyan-300 font-bold text-[11px] inline-flex items-center gap-1 cursor-pointer"
                          >
                            <RefreshCw className={`w-3.5 h-3.5 ${reconcilingId === gwTxn.traceno ? 'animate-spin' : ''}`} />
          ...
          <div className="bg-[#161b22] border border-gray-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="bg-[#0d1117] p-3 rounded-xl border border-gray-800 mb-4 text-xs space-y-1.5 font-mono">
              <div className="flex justify-between font-sans">
                ...
                <span className="font-bold text-white">{payoutItem.bankDetails?.holderName}</span>
              </div>
            ...
            <div className="mb-4">
              <label className="block text-xs font-semibold text-gray-300 uppercase mb-1.5">
                Bank Payout Reference / IMPS RRN / UTR
              </label>
              <input
                type="text"
                value={bankRefNo}
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer"
              >
